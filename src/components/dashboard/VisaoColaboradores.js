        ...
        const carregarContagens = async () => {
            try {
                ...
                for (const s of soft) {
                    ...
                    if (typeof id === 'number')
                        sMap.set(id, (sMap.get(id) || 0) + 1);
                    if (typeof id === 'number' && s?.nomeCompetencia) {
                        const set = sNames.get(id) || new Set();
